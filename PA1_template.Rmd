...
```
##### The data has `r num_obs` observations, compared with the 17,568 observations identified in the instructions.
...
```{r num_steps_hist, message=FALSE}
...
```
##### The mean and median number of steps per day are `r mean_steps_day` and `r median_steps_day` respectively.
...
##### On average, the five minute interval beginning at `r max_avg_num_steps` (military time) contains the most steps.
...
```
##### The total number of missing steps is  `r missing_steps`.
...
```{r missing_steps_by_day_of_week}
activity_data_clean %>%
  filter(is.na(steps)==T) %>%
...
```
##### The mean and median number of steps per day using the imputed data are `r imp_mean_steps_day` and `r imp_median_steps_day` respectively.  
...
```{r impute_steps_per_interval_lineplot}
